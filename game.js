...
function tryAudio(pauseOrPlay, indexOrInner, countNumber) {
  ...
  if (countNumber != 0) {
    ...
    if (indexInner == 1 && innerAudioPlayingflag == false) return
  } else {
...
//------------------------------------------------------
// Реализация встроенного окна логов (Истории) через класс .history
function showHistory() {
  ...
  var historyHtml = '<div class="history-padding"><div class="history-overflow" id="history-overflow">';
  for (var i = 0; i < gameLogHistory.length; i++) {
    var item = gameLogHistory[i];
    if (item.name) {
      historyHtml += '<div class="history-text" style="margin-bottom:0.5rem;"><span style="color:' + item.color + '; font-weight:bold;">' + item.name + '</span><br/><span style="color:#ffffff;">' + item.text + '</span></div>';
    } else {
      historyHtml += '<div class="history-text" style="margin-bottom:0.5rem; color:#cccccc;">' + item.text + '</div>';
    }
  }
  if (gameLogHistory.length === 0) {
     historyHtml += '<div class="history-text" style="text-align:center; color:#888; margin-top:2rem;">История пуста...</div>';
  }
  historyHtml += '</div></div>';
  
  $('.history').html(historyHtml);

  var logContent = document.getElementById('history-overflow');
  if (logContent) {
...
$(function () {
  ...
  $(document).on('click', '.home_btn', function(e) {
    e.stopPropagation();
    if ($(this).hasClass('home_btn_remark')) {
      hideremark();
    } else if ($(this).hasClass('home_btn_history')) {
      hideHistory();
    } else {
      ...
      if (
        ...
        $('.background')
          ...
          .css(
            'background',
            "url('" +
            'https://act-webstatic.mihoyo.com/event_bh3_com/avg-anti-entropy/static_CN/resources/background/' +
    ...
    case 'show':
      ...
      if (chara) {
        if (characterData[chara]) {
          var url =
            "url('" +
            'https://act-webstatic.mihoyo.com/event_bh3_com/avg-anti-entropy/static_CN/resources/chara/' +
...
function check_size() {
  ...
  $('html').css('font-size', (frameWidth / 736) * 16.1 + 'px');
